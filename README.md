...
- brique 2 : agent modérateur qui détecte les tentatives d'injection de prompt
...
- `src/moderator.py` : classe `Moderator` qui appelle un modèle de modération Groq en JSON
- `src/rag.py` : classe `RAG` qui orchestre la modération, le retrieval et l'appel Groq
...
## Exécution de la brique 2

Pour tester l'agent modérateur seul :

```powershell
& .venv\Scripts\python.exe -m src.moderator
```
...
- `src/moderator.py` envoie la question au modèle de la famille `safeguard`.
- Le modérateur renvoie un JSON strict `{"is_prompt_injection": true/false}`.
- `src/rag.py` refuse automatiquement la requête si le verdict indique une injection.
